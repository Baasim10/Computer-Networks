...
//#define PORT 51722
#define LISTEN_PORT 34567  // Port number for Process J 3
...
int sock_raw;
    ...
    saddr.sin_addr.s_addr = inet_addr("127.0.0.4");  //  // INADDR_ANY
 ...
 //printf("Process 3 is listening on port %d\n", LISTEN_PORT);
...
while (1) {
 ...
 /*while (1) {
        ssize_t data_size = recv(sock_raw, buffer, BUFFER_SIZE, 0);
        if (data_size < 0) {
...
fd_set read_fds;
    ...
    int n = select(sockfd + 1, &read_fds, NULL, NULL, NULL);
    if (n < 0) {
        perror("select");
        exit(EXIT_FAILURE);
    }
